...
@hook.on_start()
def load_optout(db):
    """load a list of channels monsterhunt should be off in. Right now I am being lazy and not
...
def game_opt_out(text, chan, db, conn):
    """Running this command without any arguments displays the status of the current channel. hunt_opt_out add #channel will disable all hunt commands in the specified channel. hunt_opt_out remove #channel will re-enable the game for the specified channel."""
    if not text:
        if chan in opt_out:
...
def merge_score(text, conn, db, message):
    """Moves the rpg scores from one nick to another nick. Accepts two nicks as input the first will have their scores removed the second will have the first score added. Warning this cannot be undone."""
    if freeze > 0:
        return
