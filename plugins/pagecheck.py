...
def isup(text):
    ...
    if "not just you" in content:
        return "Some people are going to be very unhappy. {} looks \x02\x034down\x02\x0f from here!".format(url)
    elif "is up" in content:
        return "Good news, the site appears to be working fine . {} is \x02\x033up\x02\x0f.".format(url)
    else:
        return "WTF? Are you sure that site even exists on the interweb?"
