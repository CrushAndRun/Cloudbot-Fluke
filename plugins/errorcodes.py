...
# Beginning phrase.
ecodeslistreply = "List of site error codes are: "
# Make the list of entries.
ecodesactuallist = list(ecodeslist.keys())
# Alphabetize it.
ecodesactuallist.sort()
# Generate the reply.
for i in ecodesactuallist:
        # Probably a better way of handling this... Don't need a comma at the start.
        if ecodeslistreply.endswith(": ") == True:
            ecodeslistreply = ecodeslistreply+i
...
def ecodes(text, nick, chan, conn):
        ...
        if text:
                ...
                try:
                        reply = ecodeslist[command]
                except KeyError:
                        reply = ecodeslistreply
                # Category doesn't exist:
                if reply == ecodeslistreply:
